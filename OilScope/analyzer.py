...
# 단위 정규화
# ---------------------------------------------------------------------------
# 밀도가 g/cm3("0.7926" 스타일)와 kg/m3("792.6" 스타일) 중 어느 쪽으로 들어오든
# 자동 판별해 g/cm3로 통일하기 위한 임계값. 석유제품 밀도는 g/cm3 기준 대략
# 0.6~1.1, kg/m3 기준으로는 600~1100 범위라 그 사이 어디를 잡아도 안전하지만,
# 10으로 잡아 두면 오차/이상치가 섞여도 오판할 여지가 거의 없다.
_DENSITY_KG_M3_THRESHOLD = 10.0


def normalize_density_g_cm3(value: float) -> float:
    """밀도 값을 g/cm3 기준으로 정규화한다.

    시험성적서/엑셀/수기입력 등 서로 다른 출처의 자료는 밀도를 g/cm3(예:
    0.7926)로 적어두기도 하고 kg/m3(예: 792.6)로 적어두기도 한다. 값 자체의
    크기로 단위를 판별해서(threshold보다 크면 kg/m3로 보고 1000으로 나눔)
    어느 쪽으로 들어오든 항상 g/cm3로 통일한다. 이 앱의 물성 혼합식(mix_density,
    MixRatioEstimator 등)은 전부 g/cm3를 전제로 하므로, 값이 시스템에 들어오는
    지점(엑셀 가져오기, JSON 가져오기, GUI 수동 입력)마다 이 함수를 거치게 한다.
    """
    if value is None:
        return 0.0
    v = float(value)
    return v / 1000.0 if abs(v) > _DENSITY_KG_M3_THRESHOLD else v
